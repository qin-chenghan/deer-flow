...
File-backed memory now isolates data by the exact `(user_id, agent_name, project_id)` scope. Each fact is canonically stored as one Markdown file with YAML front matter, while the scope's `memory.json` contains summaries plus a revisioned fact manifest. Journaled multi-file commits, scope locks, hashes, and optimistic revisions prevent silent lost updates; legacy JSON fact arrays are migrated on first access. `thread_id` remains source provenance and does not create a storage bucket. Retrieval engines stay optional and are connected through the memory retrieval-adapter contract; without one, search reports and uses the built-in substring fallback.
