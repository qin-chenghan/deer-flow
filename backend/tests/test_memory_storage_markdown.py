...
def test_project_scope_uses_internal_project_bucket(storage: FileMemoryStorage, tmp_path: Path) -> None:
    assert storage.save(_memory_with_fact("A"), user_id="alice", project_id="prj-a")
    assert storage.save(_memory_with_fact("B"), user_id="alice", project_id="prj-b")

    assert storage.load(user_id="alice", project_id="prj-a")["facts"][0]["content"] == "A"
    assert storage.load(user_id="alice", project_id="prj-b")["facts"][0]["content"] == "B"
    assert (tmp_path / "users" / "alice" / "projects" / "prj-a" / "memory.json").exists()
...
def test_thread_id_is_source_only_not_storage_bucket(storage: FileMemoryStorage) -> None:
    ...
    assert storage.save(_memory_with_fact(), user_id="alice", project_id="prj-a")
    manifest_path = storage._get_memory_file_path(user_id="alice", project_id="prj-a")
...
def test_removed_fact_is_physically_deleted(storage: FileMemoryStorage) -> None:
    ...
    assert storage.save(empty, user_id="alice", project_id="prj-a")
...
def test_storage_delegates_index_lifecycle_and_search_to_retrieval(tmp_path: Path) -> None:
    ...
    assert scoped.save(_memory_with_fact(), user_id="alice", project_id="prj-a")
    assert retrieval.upserts[0][1]["projectId"] == "prj-a"
    assert scoped.search_facts("python", scopes=[{"userId": "alice", "agentName": None, "projectId": "prj-a"}])[0]["score"] == 0.9

    assert scoped.save(create_empty_memory(), user_id="alice", project_id="prj-a")
    assert retrieval.removes == [("fact_01HZZZZZZZZZZZZZZZZZZZZZZZ", {"userId": "alice", "agentName": None, "projectId": "prj-a"})]
...
def test_fact_repository_applies_upsert_and_physical_delete(storage: FileMemoryStorage) -> None:
    first = storage.upsert_fact(_memory_with_fact()["facts"][0], user_id="alice", project_id="prj-a", expected_revision=0)
    assert first["revision"] == 1
    assert storage.get_fact("fact_01HZZZZZZZZZZZZZZZZZZZZZZZ", user_id="alice", project_id="prj-a")["content"] == "Project uses Python 3.12"
    ...
    assert fact_path.exists()
    third = storage.delete_fact(updated["id"], user_id="alice", project_id="prj-a", expected_revision=2)
    assert third["facts"] == []
    assert not fact_path.exists()


def test_search_facts_declares_and_uses_substring_fallback(storage: FileMemoryStorage) -> None:
    storage.upsert_fact(_memory_with_fact()["facts"][0], user_id="alice", project_id="prj-a", expected_revision=0)

    results = storage.search_facts(
        "python",
        scopes=[{"userId": "alice", "agentName": None, "projectId": "prj-a"}],
    )
