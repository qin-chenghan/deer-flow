...
def test_project_id_is_part_of_queue_identity_and_forwarded() -> None:
    mock_updater = MagicMock()
    mock_updater.update_memory.return_value = True
    queue = _queue(mock_updater)
    with patch.object(queue, "_reset_timer"):
        queue.add(thread_id="thread-1", messages=["A"], user_id="alice", project_id="prj-a")
        queue.add(thread_id="thread-1", messages=["B"], user_id="alice", project_id="prj-b")

    assert len(queue._queue) == 2
    queue._process_queue()
    assert {call.kwargs["project_id"] for call in mock_updater.update_memory.call_args_list} == {"prj-a", "prj-b"}
