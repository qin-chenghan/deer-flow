...
- `queue.py` - Debounced update queue (per exact thread/user/agent/project identity, configurable wait time); captures the full scope before crossing the `threading.Timer` boundary
- `prompt.py` - Prompt templates for memory updates
- `storage.py` - File repository with single-fact Markdown, JSON manifests/summaries, journal recovery, file locks, revisions/hashes, deep-copy caching, and an optional retrieval adapter
...
**Scope Isolation**:
- Canonical scope is `(user_id, agent_name, project_id)`; `thread_id` is fact source provenance only and never a storage partition
- User memory manifest is `{base_dir}/users/{user_id}/memory.json`; an optional project adds `/projects/{project_id}/memory.json`
- Per-agent memory is `{base_dir}/users/{user_id}/agents/{agent_name}/memory.json`; an optional project again adds `/projects/{project_id}/memory.json`
- Canonical facts are sharded under the same bucket at `facts/{id-prefix}/{fact-id}.md`; `memory.json` stores summaries and each fact's relative path, revision, and content hash
- Custom agent definitions (`SOUL.md` + `config.yaml`) are also per-user at `{base_dir}/users/{user_id}/agents/{agent_name}/`. The legacy shared layout `{base_dir}/agents/{agent_name}/` remains read-only fallback for unmigrated installations
- Middleware and tool modes propagate the runtime's internal `project_id` with `user_id` and `agent_name`; subagents, manual compaction, prompt injection, and Gateway memory routes preserve the same scope
- Gateway project-scope validation goes through `MemoryManager.validate_project_id()`; app/router code must not import a backend's private path or schema helpers
...
- **Facts**: schema-v2 Markdown documents with `id`, content, core category plus optional extension category, topics, confidence, status, exact scope, structured source, timestamps, revision, and consolidation provenance
- **Repository**: `get/list/upsert/delete_fact`, `apply_changes`, summary operations, explicit migration, index notifications/rebuild/status, and scoped search; legacy whole-document `load/save` remains as a compatibility layer
...
- Both modes share `FileMemoryStorage`, exact scope isolation, prompt injection, manual CRUD primitives, and the updater backend.
- Middleware mode queue debounces (30s default), batches updates, and submits one change set through the journaled repository path. File commits use per-scope in-process/cross-process locks and optimistic manifest revisions; a crash is recovered from the operation journal on the next access.
- A configured `retrieval_adapter` owns indexing and semantic retrieval. File storage only sends idempotent upsert/remove notifications and delegates search; without an adapter, its declared `substring_fallback` capability keeps local search operational.
...
- `storage_path` - DeerMem storage root; manifests and Markdown facts are created below scoped user/agent/project buckets
- `storage_class` - `file` or a dotted `MemoryStorage` class; invalid persistent backends fail fast
- `strict_user_scope` - Require `user_id` for all memory access (default `false` for no-auth/legacy compatibility)
...
- `retrieval_adapter` - Optional dotted factory receiving `DeerMemConfig`; its result implements the retrieval port
