...
- Middleware mode queue debounces (30s default), batches updates, and commits through a user-level lock, optimistic user-memory revisions, per-fact revisions, and a recoverable target-file journal. A stale shared revision may be retried at most three times only for fact-only changes whose target revisions still match; same-fact conflicts and summary conflicts fail. Agent writes may change only that agent's facts; only `agent_name=None` writes may change global summaries.
