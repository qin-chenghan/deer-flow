...
> 状态：File/Markdown storage 已按本方案实施；检索算法及最终联调签名仍待 retrieval 负责人锁定。实施位置见 [`STORAGE_REWRITE_CHANGES.md`](STORAGE_REWRITE_CHANGES.md)。  
...
但它还不能满足即将接入的检索模块和多项目、多 worker 场景。此次改写目标是：

1. 建立统一、不可绕过的 scope 模型，阻止 user、agent、thread、project 间的记忆混用；
...
```text
MemoryContext = (user_id, agent_name, thread_id, project_id)
```
...
| `project_id` | 是 | `None` 表示用户/agent 公共层；非空表示项目层；使用服务端内部稳定 ID |

其中真正的长期记忆隔离键为：

```text
MemoryScope = (user_id, agent_name, project_id)
```
...
当前项目中的一次查询，不应只读取一份库，而应按层合并：

1. 用户公共：`(user, None, None)`；
2. 用户当前项目：`(user, None, project_id)`；
3. 当前 agent 公共：`(user, agent, None)`；
4. 当前 agent + project：`(user, agent, project_id)`；
5. `thread_id` 不形成单独长期记忆层，只作为候选的来源过滤条件（如确有业务需要）。

优先级建议为“精确 scope 高于公共 scope”，但 correction 等 guaranteed category 的最终策略归 injection 所有。Storage 只负责精确过滤和返回候选，不负责 token 截断。

### 5.3 `None` 的风险控制

- `project_id=None` 必须明确表示“公共记忆”，不能同时表示“调用方忘了传 project”。
- 动态项目功能启用后，项目型 fact 若缺 `project_id`，应在 write gate 被拒绝或降为待人工确认。
- 生产模式默认禁止 `user_id=None` 写入；legacy 模式需显式配置开启。
- scope 必须写入 fact/DB 记录，即使目录已经隐含 scope，防止导入、移动和重建索引时丢失上下文。

### 5.4 动态 project 与 ABC 裁决

已决定支持动态 project，并允许修改 `MemoryManager` ABC。不能再用 `backend_config.project` 模拟动态项目。

本轮需要：

- 在 ABC 的 memory 相关方法中增加 keyword-only `project_id: str | None = None`；
- `thread_id` 已存在的写入方法继续用于 source；读取/CRUD 如需溯源过滤可按具体接口增加 keyword-only 参数；
- Gateway/runtime/tool/middleware/subagent 从服务端可信上下文传播内部 `project_id`；
- DeerMem、noop 和所有其他 backend 同步签名；
- 未传 `project_id` 时只表示公共 scope，不能从目录名或用户文本猜测；
- project 授权由 host 完成，core 只接收已验证的内部 ID。

这是一项明确的跨插件契约变更，需与 retrieval/injection/updater 同版本协调。
...
```json
{
  "id": "fact_01...",
  "schemaVersion": 2,
  "content": "DeerFlow memory storage 需要 project 隔离",
  "title": "Memory storage project isolation",
  "category": "context",
...
2. project 使用服务端内部稳定 `projectId`，显示名不作为隔离键。
3. category 采用“核心枚举 + 扩展类别”：
...
4. 第一版不在每次读取时同步写 `lastAccessedAt`，避免检索产生写放大；字段可保留为空，后续由 retrieval 模块决定是否异步聚合。
5. 删除采用彻底物理删除：canonical Markdown、manifest 条目及检索索引均删除。为了可恢复和审计，删除前写 operation journal/audit，但不保留可被正常读取的 tombstone fact。
6. `source` 迁移为对象，同时 reader 接受旧字符串。
7. `metadata` 必须设置允许键策略和大小上限，不能替代正式 schema 字段。
...
# Memory storage project isolation

DeerFlow memory storage 需要按 project 隔离项目事实。
```
...
1. 按已定决策修改 ABC/runtime，端到端传播动态内部 `project_id`；
2. 锁定 `MemoryScope=(user_id, agent_name, project_id)` 和 source `thread_id` 语义；
3. 将已接受的 fact schema v2 固化为机器可校验 schema；
4. 与 retrieval 负责人锁定委托 API 签名、错误语义和返回类型；
5. 建立当前 File 行为的回归测试；
6. 建立并发 lost-update 复现测试；
7. 记录现有 JSON fixture 和迁移样本。
...
**退出条件**：零配置行为兼容；跨 user/agent/project/thread 测试通过；损坏数据不会被覆盖。
...
3. 传递内部 `project_id` 和其他 scope 过滤条件；
...
### Phase 5：动态 project 管理 API 与前端配合
...
- project A 永远检索不到 project B 的项目 fact；
- agent A/B 按分层规则隔离；
...
| 风险/冲突 | 裁决 |
|---|---|
| project 加入 scope | 已决定修改 ABC/runtime，传播可信内部 `project_id` |
...
8. 动态 project ABC/runtime 接线；
9. 并发、安全、迁移、索引和端到端测试；
10. README、相关 AGENTS.md、config.example.yaml 更新。

## 19. 已确认决策与剩余联调项

### 19.1 已确认

- [x] project 为每个 thread/run 动态值；
- [x] 允许修改 MemoryManager ABC/runtime context；
- [x] thread_id 只作来源；
